...
const communeWithQuarantine = ['SANTIAGO','MAIPU','RANCAGUA','VALPARAISO','COQUIMBO'];
...
describe('Validate parameters', () => {
  ...
  test('Parameter hr, must be a number between 0 and 23 and return true', () => {
    ...
    expect(calculate(mockObjValid.hr, mockObjValid.place, mockObjValid.weekDay)).toBe(true);
  });
})

//  describe('Validate quarantine place', () => { 
//   test('If place are in quarantine response false', () => {
//     const mockCallback = jest.fn(() => communeWithQuarantine.includes(mockObjNotValid.place));
//     expect(mockCallback()).toBe(true);
//     expect(calculate(15, mockObjNotValid.place, mockObjNotValid.weekDay)).toBe(false);
//   });

//   test('If place are in quarantine response true', () => {
//     const mockCallback = jest.fn(() => communeWithQuarantine.includes(mockObjValid.place));
...
describe('Validate quarantine place', () => { 
    test('If place are in quarantine response false', () => {
      const isQuarantinePlace = () => communeWithQuarantine.includes(mockObjNotValid.place);
      expect(isQuarantinePlace()).toBe(true);
      expect(calculate(validHr, mockObjNotValid.place, validDay)).toBe(false);
    });

    test('If place are in quarantine response true', () => {
      const isQuarantinePlace = () => communeWithQuarantine.includes(mockObjValid.place);
...
describe('Validate function and response true or false according to implementation ', () => {
  ...
  test('Validate incorrect', () => {
    expect(calculate(notValidHr, mockObjNotValid.place, mockObjNotValid.weekDay)).toBe(false);
  });
});
